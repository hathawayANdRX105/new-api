...
type RouteKey struct { ChannelId int; Model string }
...
type routeHealthState struct { State string; IsolationLevel int; Until *int64; Version int; DormantDisableCount int }
...
func IsRouteHealthy(key RouteKey, now time.Time) bool {
	routeHealthLock.RLock(); state := routeHealthIDM[key]; routeHealthLock.RUnlock()
	if state == nil || state.State == HealthHealthy { return true }
	if state.State == HealthDisabled { return false }
	return state.Until == nil || *state.Until <= now.Unix()
}
func GetRouteHealth(key RouteKey) (string, bool) {
	routeHealthLock.RLock(); state := routeHealthIDM[key]; routeHealthLock.RUnlock()
	if state == nil { return HealthHealthy, true }
	return state.State, state.State == HealthHealthy
}
func cacheHealth(row *ChannelModelHealth) {
	var until *int64
	if row.Until != nil { v := *row.Until; until = &v }
	routeHealthLock.Lock(); routeHealthIDM[RouteKey{row.ChannelId, row.Model}] = &routeHealthState{row.State,row.IsolationLevel,until,row.Version,row.DormantDisableCount}; routeHealthLock.Unlock()
}
func ClearRouteHealthCache() { routeHealthLock.Lock(); routeHealthIDM = map[RouteKey]*routeHealthState{}; routeHealthLock.Unlock() }

func isolationDuration(level int, cfg *operation_setting.ChannelModelHealthSetting) (string, int64) {
	switch {
	case level <= 0: return HealthHealthy, 0
	case level <= 3: return HealthCalm, int64(cfg.CalmFastBase+(level-1)*cfg.CalmFastInterval)
	case level <= 6: return HealthCalm, int64(cfg.CalmSlowBase+(level-4)*cfg.CalmSlowInterval)
	case level <= 9: return HealthDormant, int64(cfg.DormantBase+(level-7)*cfg.DormantInterval)
	default: return HealthDormant, int64(cfg.DormantMaxBase)
	}
}
...
func RecordRetryableFailure(key RouteKey, errorCode string, now time.Time) error {
	...
	for attempt := 0; attempt < 3; attempt++ {
		...
		if err := DB.Where("channel_id = ? AND model = ?", key.ChannelId, key.Model).First(&row).Error; err != nil {
			...
			if err := DB.Create(&row).Error; err != nil { continue }
		}
		...
		if row.State == HealthDormant && row.Until != nil && *row.Until <= now.Unix() {
			...
			if cfg.DormantDisableThreshold > 0 && row.DormantDisableCount >= cfg.DormantDisableThreshold { state, seconds = HealthDisabled, 0 }
		}
		until := (*int64)(nil)
		if state != HealthDisabled { deadline := now.Unix() + seconds; until = &deadline }
		...
		if q.Error != nil { return q.Error }
		if q.RowsAffected == 0 { continue }
...
func RecoverRoute(key RouteKey, now time.Time) error { return updateRouteState(key, HealthHealthy, 0, nil, 0, now) }
func DisableRoute(key RouteKey, now time.Time) error { return updateRouteState(key, HealthDisabled, 0, nil, 0, now) }
