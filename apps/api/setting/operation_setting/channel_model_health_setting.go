...
func GetChannelModelHealthSetting() *ChannelModelHealthSetting { return channelModelHealthSetting.Load() }
...
func ValidateChannelModelHealthSettingValue(key, value string) error {
	if !IsChannelModelHealthOptionKey(key) { return fmt.Errorf("unknown channel model health option %q", key) }
	v, err := strconv.Atoi(value)
	if err != nil || v < 0 { return fmt.Errorf("%s must be a non-negative integer", key) }
	return nil
}

func UpdateChannelModelHealthSettingValue(key, value string) error {
	if err := ValidateChannelModelHealthSettingValue(key, value); err != nil { return err }
	v, _ := strconv.Atoi(value)
	old := channelModelHealthSetting.Load()
	next := *old
	switch key {
	case "CalmFastBase": next.CalmFastBase = v
	case "CalmFastInterval": next.CalmFastInterval = v
	case "CalmSlowBase": next.CalmSlowBase = v
	case "CalmSlowInterval": next.CalmSlowInterval = v
	case "DormantBase": next.DormantBase = v
	case "DormantInterval": next.DormantInterval = v
	case "DormantMaxBase": next.DormantMaxBase = v
	case "DormantDisableThreshold": next.DormantDisableThreshold = v
	}
